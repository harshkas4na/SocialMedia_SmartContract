...
contract Governance is Ownable {
    ...
    function createProposal( string memory description,uint256 _voteThreshold,uint256 _subgroupId) external {
        proposalCount++;
    ...
    function vote(  uint256 proposalId, bool support) external {
        ...
        if(proposals[proposalId].votesFor >= proposal.voteThreshold){
            emit ProposalForThresholdReached(proposalId);
        }
        else if(proposals[proposalId].votesAgainst >= proposal.voteThreshold){
